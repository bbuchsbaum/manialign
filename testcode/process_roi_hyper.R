...
load_vdat <- function(reduce=TRUE, ROI=NULL) {
  lapply(sids, function(sid) {
    ...
    red <- function(mat) {
      gnums <- rep(1:(nrow(mat)/4), each=4)
      gmat <- group_means(ordered(gnums), mat)
    }
    ...
    v2mat <- red(v2$mat)
    v3mat <- red(v3$mat)
    des1 <- data.frame(video="video_1", sid=sid, time=rownames(v1mat))
    des2 <- data.frame(video="video_2", sid=sid, time=rownames(v2mat))
    des3 <- data.frame(video="video_3", sid=sid, time=rownames(v3mat))

    des <- rbind(des1, des2, des3)
    des$run <- factor(rep(1:3, each=nrow(des1)))
    mat <- rbind(v1mat, v2mat, v3mat)

    if (!is.null(ROI)) {
      keep <- apply(mat,2, function(x) sum(x==0)) == 0
      keep <- keep & (v1$roi %in% c(ROI, ROI+1000))
    } else {
      keep <- apply(mat,2, function(x) sum(x==0)) == 0
    }
    ...
    list(sid=sid, mat=rmat, design=des, keep=keep, roi=roi, nodes=nodes)
  })
}



load_ndat <- function(vdat=NULL, ROI=NULL) {
  lapply(1:length(sids), function(i) {
    ...
    mat <- group_means(v1$design$Label, mat)
    ...
    list(sid=sid, mat=mat_miss, design=des, keep=keep, len=ncol(mat), roi=roi, nodes=nodes)
  })
}
...
load_idat <- function(rnums) {
  lapply(ndat, function(vd) {
    ...
    if (use_s5) {
      ...
      v1 <- load_mat(paste0(sid, "_ventral_imagery.RDS"))
    }
    ...
    list(sid=sid, spmat=spmat, design=v1$design, keep=keep, len=ncol(mat), roi=vd$rois, nodes=vd$nodes)
...
if (use_video) {
  vdat <- load_vdat(reduce=TRUE, ROI=11121)
  ndat <- load_ndat(vdat)
  idat <- load_idat(ndat)
} else {
  ndat <- load_ndat()
